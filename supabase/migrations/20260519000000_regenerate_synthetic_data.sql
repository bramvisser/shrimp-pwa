-- Regenerate synthetic measurement + mortality data so the dashboards have
-- fresh content ending at "now". The previous seed migration ran on
-- 2026-03-26, anchored at now() - 14 weeks, so its newest row is ~7 weeks
-- stale. This migration wipes both tables and runs the same generator
-- anchored at the time this migration is applied.
--
-- Wipe scope: both tables, including any real submissions. (Decision: this
-- is a demo dataset; we don't preserve user-entered rows.)

TRUNCATE TABLE measurements, mortalities RESTART IDENTITY CASCADE;
...
DECLARE
  ...
  -- Tank assignments per farm index (1-based)
  ...
  FOR f_idx IN 1..array_length(farm_slugs, 1) LOOP
    ...
    FOR t_idx IN 1..tank_count LOOP
      ...
      FOR w IN 0..13 LOOP
        ...
        -- higher in early weeks; one tank gets a mid-cycle disease spike.
